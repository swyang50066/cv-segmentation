...
class LevelSet(object):
    ...
    def getGradientIndicator(self, x, alpha=2, sigma=1):
    ...
    def calcCurvature(self, phi, eta=1.e-8):
    ...
    def run(self, image, seed):
        ...
        indicator = self.getGradientIndicator(image)

        # Calculate gradient of indicator  
        fx, fy = self.calcGradient(indicator)

        # Optimization
        for i in range(self.maxIter):
            # Calculate gradient of level-set
            gx, gy = self.calcGradient(phi)
            grad = np.sqrt(gx**2. + gy**2.)

            # Calculate curvature
            kappa = self.calcCurvature(phi)
