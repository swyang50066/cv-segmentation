...
def convertArrayToVTKArray(array, dtype=None):
    ...
    vtkArray.SetVoidArray(contiguous,
                          contiguous.size, 
                          1)
...
class IsoSurface(object):
    ...
    def run(self, volume, renderer):
        ...
        self._setup_data(np.float32(self.volume))
        self._image_data.SetSpacing(self.spacing[2], self.spacing[1], self.spacing[0])
    ...
    def _setup_data(self, volume):
        ...
        self._volume_array = np.zeros(volume.shape, dtype="float32", order="C")
        ...
        self._float_array.SetVoidArray(self._volume_array, np.product(volume.shape), 1)
    ...
    def set_volume(self, volume):
        ...
        self._volume_array[:] = volume
        self._float_array.SetVoidArray(self._volume_array, np.product(self._volume_array.shape), 1)
    ...
    def set_cmap(self, cmap):
        ...
        self._mapper.SetLookupTable(getColormap(self._volume_array.min(), self._volume_array.max(), colorscale=cmap))
        self._render()
...
class IsoLine(object):
    ...
    def run(self, curve, renderer):
        # >>>> STL Rendering
        points = vtk.vtkPoints()
        for center, _ in curve:
            points.InsertNextPoint((center[2], 
                                    center[1], 
                                    center[0]))
